...
export default function ReactionForm() {
  const { register, handleSubmit, reset, setValue, watch} = useForm<ReactionFormData>();
  ...
  return (
    <div className="max-w-lg mx-auto mt-8 p-4 bg-white rounded shadow">
      <h1 className="text-xl font-semibold mb-4">{uuid?'Edit Reaction': 'Create Reaction'}</h1>
      <form onSubmit={handleSubmit(onSubmit)}>
        <label className="block mb-2">
          ...
          watch("type_video") === VideoTypeEnum.SourceVideo.toString() && (
            ...
            <UploadVideo 
              folder="videos"
              onUploadComplete={(fileName: string, fileUrl: string, fileDuration: number) => {
                console.log("File uploaded:", fileName, fileUrl, "Duration:", fileDuration);
